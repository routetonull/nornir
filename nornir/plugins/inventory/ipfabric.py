        ...
        **kwargs: Any,
        ...
        Arguments:
            ipf_url: IP Fabric url, defaults to http://localhost:8080.
                You can also use env variable IPF_URL
        ...
        credentials = b64encode(f'{ipf_user}:{ipf_password}'.encode('utf-8')).decode('utf-8')
        headers = {'Content-type': 'application/json', 'Authorization': f'Basic {credentials}'}

        data = {
            'columns' : ["sn","hostname","siteName","loginIp","loginType","vendor","platform","family","version"],
            'filters' : {},
            'snapshot' : ipf_snapshot,
        } 
        r = requests.post(url, data=json.dumps(data),headers=headers,verify=ssl_verify)
        ...
        for d in ipf_devices:
            ...
            host["groups"] = [d.get('siteName'),d.get('platform'),d.get('family') or d.get("vendor"),d.get('vendor')]
            ...
            # set napalm platform
            host["connection_options"]= {"napalm" : { "platform" : napalm_platform_map.get(d["family"],d["family"])}}
